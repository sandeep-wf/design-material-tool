...
def navigate(s): st.session_state.screen = s; st.rerun()

st.sidebar.image(W_LOGO, use_container_width=True)
st.sidebar.title('Navigation')
cnt = sum(i['quantity'] for i in st.session_state.cart.values())
if st.sidebar.button(f'🛒 View My Cart ({cnt})', use_container_width=True): navigate('Cart Management')
if st.sidebar.button('🔍 Start New Search', use_container_width=True): navigate('Design Selection')
...
if st.session_state.screen == 'Design Selection':
    ...
    if st.button('🔙 Back to Search'): navigate('Design Selection')
    ...
    if st.button('➕ Add More Items'): navigate('Design Selection')
