...
# --- Sidebar Navigation ---
st.sidebar.title("🛠️ Selection Tool")
cart_count = sum(item['quantity'] for item in st.session_state.cart.values())

if st.sidebar.button(f"🛒 View Cart ({cart_count})", use_container_width=True):
    navigate_to('Cart Management')

if st.sidebar.button("🔍 Search Designs", use_container_width=True):
    navigate_to('Design Selection')
